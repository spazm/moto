...
class CognitoIdpUser(BaseModel):
    ...
    def to_json(
        self, extended=False, attributes_key="Attributes", attributes_to_get=[]
    ):
        ...
        if attributes and attributes_to_get:
            attributes = [
                attr for attr in attributes if attr["Name"] in attributes_to_get
            ]
        if extended:
            user_json.update(
                {
